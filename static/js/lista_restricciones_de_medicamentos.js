$.getScript("/static/js/datatables.spanish.js", function() {    
   $(document).ready(function() {
        ...
        $('#miTabla').on('click', '#editar', function() {
            let idRestMedic = $(this).data('id');
            cargarInformacionRestriccionMedicamento(idRestMedic);
        });
        ...
        // Funcion para registrar una nueva restriccion
        function registrarRestriccionMedic(form) {
            var formData = $(form).serialize();
            $.ajax({
                url: "registrarRestriccionMedicamento/",
                type: "POST",
                data: formData,
                headers: { "X-CSRFToken": $("input[name=csrfmiddlewaretoken]").val() },
                success: function(response) {
                    $("#modal-registrar-restriccion").modal("hide");
                    if (response.success === true) {
                        ...
                        if (response.errors && response.errors.nombre) {
                            ...
                            Swal.fire({
                                title: 'Error',
                                text: 'Ocurrió un error al registrar la restricción de medicamento.',
                                icon: 'error'
                            });
                        }
                    }
        ...
        $("#registroRestriccionMedicForm").validate({
            rules: {
                nombre: {
                    required: true,
                    minlength: 3,
            ...
            submitHandler: function (form) {
                registrarRestriccionMedic(form);
                return false;
            },
        });

        $("#modal-registrar-restriccion").on("hidden.bs.modal", function () {
            if (registroSuccessful) {
                Swal.fire({
                    title: 'Éxito',
                    text: 'La restricción de medicamentos fue registrada correctamente.',
                    icon: 'success'
                });
                registroSuccessful = false;
            }
            // Limpiar el formulario del modal de registro
            $('#registroRestriccionMedicForm')[0].reset();
            $('#registroRestriccionMedicForm').find('.is-invalid').removeClass('is-invalid');
            $('#registroRestriccionMedicForm').find('.invalid-feedback').remove();
        });

        $('#registrarRestriccionMedicButton').on('click', function() {
            $('#modal-registrar-restriccion').modal('show');
        });
