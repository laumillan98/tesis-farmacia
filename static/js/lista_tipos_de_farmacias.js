$(document).ready(function() {
    ...
    var table = $('#miTabla').DataTable({
     ...
     function cargarInformacionTipoFarmacia(id) {
        $.ajax({
            url: 'obtenerTipoFarmacia/' + id + '/',
            type: 'GET',
            data: {},
            success: function(response) {
                $('#tipo').val(response.name);
                $('#id').val(response.id);
            }
        });
    }
    ...
    function editarTipoFarmacia(form) {
        var formData = $(form).serialize()
    
        // Enviar los datos al servidor usando AJAX
        $.ajax({
            url: "editarTipoFarmacia/",
            type: "POST",
            data: formData,
            ...
            error: function (error) {
                alert("Ocurrió un error al editar el tipo de farmacia")
            },
        })
    }


    $("#edicionTipoFarmaciaForm").validate({
        rules: {
          nombre: {
            required: true,
            minlength: 3,
    ...
    $("#modal-lg").on("hidden.bs.modal", function () {
        if (editionSuccessful) {
            Swal.fire({
                title: 'Éxito',
                text: 'El tipo de farmacia fue editado correctamente.',
                icon: 'success'
            });
    ...
    function registrarTipoFarmacia(form) {
        var formData = $(form).serialize();
        $.ajax({
            url: "registrarTipoFarmacia/",
            type: "POST",
            data: formData,
            headers: { "X-CSRFToken": $("input[name=csrfmiddlewaretoken]").val() },
            success: function(response) {
                $("#modal-registrar-tipo").modal("hide");
                if (response.success === true) {
                    ...
                    if (response.errors && response.errors.nombre) {
                        ...
                        Swal.fire({
                            title: 'Error',
                            text: 'Ocurrió un error al registrar el tipo de farmacia.',
                            icon: 'error'
                        });
                    }
                }
    ...
    $("#registroTipoFarmaciaForm").validate({
        rules: {
            nombre: {
                required: true,
                minlength: 3,
        ...
        submitHandler: function (form) {
            registrarTipoFarmacia(form);
            return false;
        },
    });

    $("#modal-registrar-tipo").on("hidden.bs.modal", function () {
        if (registroSuccessful) {
            Swal.fire({
                title: 'Éxito',
                text: 'El tipo de farmacia fue registrado correctamente.',
                icon: 'success'
            });
            registroSuccessful = false;
        }
        // Limpiar el formulario del modal de registro
        $('#registroTipoFarmaciaForm')[0].reset();
        $('#registroTipoFarmaciaForm').find('.is-invalid').removeClass('is-invalid');
        $('#registroTipoFarmaciaForm').find('.invalid-feedback').remove();
    });

    $('#registrarTipoFarmaciaButton').on('click', function() {
        $('#modal-registrar-tipo').modal('show');
    });
